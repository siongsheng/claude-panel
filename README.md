...
- **Pre-merge remote-verification** (`bin/pre-merge-check`) — local gates prove nothing about the ref that actually merges. Asserts the local `HEAD` equals the remote branch head (catching unpushed commits that would merge a PR *incomplete*), that the fix is on the remote ref and the old buggy line is gone, and — post-merge — that the default branch's test count matches the branch's. Every check is a mechanical fact, so it's a hard gate.
