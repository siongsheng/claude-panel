...
- **Mutation gate** (`bin/mutation-check`) — `tdd-check` proves commit *order*; this proves a test would actually *fail* if the code broke. Its one hard-failure mode is a structural fact, not a judgement: an **invalid run** (a mutant that never recompiled — so it silently tested the unmutated binary — or a configured self-test that wasn't caught) fails like a red `tdd-check`. A surviving mutant is *advisory* — undecidable whether it's a real gap or an equivalent mutant — so it earns a findings-ledger disposition, never a block. A diff that mutates nothing, or a repo with no configured mutation tool, skips loudly (green).
